...
class Node():
  def __init__(self, descr_string):
    self.str = descr_string
    self.weight = int(descr_string.split()[1][1:-1])
    self.totWeight = self.weight
    self.name = self.str.split()[0].strip()
    if "->" in self.str:
      self.neighbours = self.str.split("->")[-1]
      self.neighbours = [x.strip() for x in self.neighbours.split(",")]
      self.numNeighbours = len(self.neighbours)
    else:
      self.neighbours = []
      self.numNeighbours = 0
 
  def hasNeighbour(self):
    return self.numNeighbours != 0
 
  def isNeighbour(self, node2):
    return node2.name in self.neighbours
 
  def display(self):
    print "NODE:", self.name, " totWEIGHT:", self.totWeight, "weight", self.weight
 
def isANeighbour(node, graph):
  for node2 in graph:
    if node2.isNeighbour(node):
      return True
  return False
...
def findNeighbours(node, graph):
  neighbour_l = []
  for node2 in graph:
    if node2.name in node.neighbours:
      neighbour_l.append(node2)
  return neighbour_l
...
def part1():
  ...
  graph = []
  for line in input_l:
      graph.append(Node(line))
 
  hasNeighbours = []
  for node in graph:
    if node.hasNeighbour():
      hasNeighbours.append(node)
 
  for node in hasNeighbours:
   if not isANeighbour(node, hasNeighbours):
     print "Node is not a neighbour:", node.str
...
def part2():
  ...
  for node in hasNeighbours:
   if not isANeighbour(node, hasNeighbours):
     rootNode = node
  rootNode.display()
  neighbour_l = findNeighbours(rootNode, graph)
  
  for nei in neighbour_l:
    nei.display()

  neighbour_l = [rootNode]
  levels_l = [[rootNode]]
  levels = 1
  level = [1]
  while level:
    level = []
    for nei in levels_l[levels-1]:
      level += findNeighbours(nei, graph)
    levels += 1
    levels_l.append(level)
  print "Levels", levels
  print
  print "Level 0"
  ind = 5
  while ind >= 0:
    ind -= 1
    for node in levels_l[ind]:
      neighbours = findNeighbours(node, graph)
      neighbour_weights = [nei.totWeight for nei in neighbours]
      if len(set(neighbour_weights)) > 1:
        print "neighbour_weights", neighbour_weights
        print "not equal"
        neighbours[2].weight -= 5
        neighbours[2].totWeight -= 5
        print "New weight", neighbours[2].weight
        neighbour_weights = [nei.totWeight for nei in neighbours]

      node.totWeight += sum(neighbour_weights)
